...
def main():
    ...
    with pd.ExcelWriter(excel_path, engine='xlsxwriter') as writer:
        ...
        pd.DataFrame(cm, index=["True_0", "True_1", "True_2", "True_3+"],
                        columns=["Pred_0", "Pred_1", "Pred_2", "Pred_3+"]).to_excel(writer, sheet_name='Confusion_Matrix')
        pd.DataFrame(report).transpose().to_excel(writer, sheet_name='Classification_Report')
    torch.save(model.state_dict(), os.path.join(save_dir, 'final_model_D_selected.pth'))
