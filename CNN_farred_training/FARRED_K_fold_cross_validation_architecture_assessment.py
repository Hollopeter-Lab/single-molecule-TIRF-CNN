...
def main():
    ...
    save_dir = f"K_fold_cross_val_Model_39_13_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    excel_filename = os.path.join(save_dir, 'training_summary.xlsx')
    with pd.ExcelWriter(excel_filename, engine='xlsxwriter') as writer:
        for fold, (train_idx, val_idx) in enumerate(skf.split(traces, labels)):
            ...
            model = StepDetectionCNN(input_length=traces.shape[1], num_classes=4).to(device)
            ...
            df_cm = pd.DataFrame(cm, index=["True_0", "True_1", "True_2", "True_3+"], columns=["Pred_0", "Pred_1", "Pred_2", "Pred_3+"])
            ...
            report = classification_report(y_true, y_pred, output_dict=True, target_names=["Class_0", "Class_1", "Class_2", "Class_3+"])
            metrics_summary = {
                'Fold': fold+1,
                ...
                'Precision Class 0': report['Class_0']['precision'],
                'Recall Class 0': report['Class_0']['recall'],
                'F1 Score Class 0': report['Class_0']['f1-score'],
                'Precision Class 1': report['Class_1']['precision'],
                'Recall Class 1': report['Class_1']['recall'],
                'F1 Score Class 1': report['Class_1']['f1-score'],
                'Precision Class 2': report['Class_2']['precision'],
                'Recall Class 2': report['Class_2']['recall'],
                'F1 Score Class 2': report['Class_2']['f1-score'],
                'Precision Class 3+': report['Class_3+']['precision'],
                'Recall Class 3+': report['Class_3+']['recall'],
                'F1 Score Class 3+': report['Class_3+']['f1-score'],
        ...
        df_mean_cm = pd.DataFrame(mean_cm, index=["True_0", "True_1", "True_2", "True_3+"], columns=["Pred_0", "Pred_1", "Pred_2", "Pred_3+"])
